...
if(!function_exists('getPriceFrontend')){
	function getPriceFrontend($param){
		...
		if(isset($productDetail['price_sale']) && $productDetail['price_sale'] != '' && $productDetail['price_sale'] != 0){
			$price_old = $productDetail['price_sale'];
		}
		...
		if(isset($jsonData) && is_array($jsonData) && count($jsonData)){
			foreach($jsonData as $key => $val){
				...
				$jsonData[$key][(count($module) == 1) ? 'post'  : 'catalogue'] = $CI->Autoload_Model->_get_where(array(
					'select' => 'id, title, slug, canonical, image, description'.((count($module) > 1) ? ', icon' : '').''.((count($module) == 1 && $module[0] == 'product') ? ', price_sale, price, code' : '').'',
					'table' => $val['module'],
					...
					if(isset($val['catalogue']) && is_array($val['catalogue']) && count($val['catalogue'])){
						foreach($val['catalogue'] as $keyChildren => $valChildren){
							$jsonData[$key]['catalogue'][$keyChildren]['children'] =  $CI->Autoload_Model->_get_where(array(
								'select' => 'id, title, icon, description, image, canonical',
								'table' => $val['module'],
								...
								if(isset($valChildren['children']) && is_array($valChildren['children']) && count($valChildren['children'])){
									foreach($valChildren['children'] as $keyPost => $valPost){
										$jsonData[$key]['catalogue'][$keyChildren]['children'][$keyPost]['post'] =  $CI->Autoload_Model->_condition(array(
											'module' => $module,
											'select' => '`object`.`id`, `object`.`title`, `object`.`slug`, `object`.`canonical`, `object`.`image`,`object`.`created` '.(($module == 'product') ? ', `object`.`price`, `object`.`price_sale`, `object`.`code`, `object`.`price_contact`' : '').(($module == 'media') ? '`object`.`video_link`, `object`.`video_iframe`' : ''),
					...
					if(isset($val['catalogue']) && is_array($val['catalogue']) && count($val['catalogue'])){
						foreach($val['catalogue'] as $keyChildren => $valChildren){
							$jsonData[$key]['catalogue'][$keyChildren]['post'] =  $CI->Autoload_Model->_condition(array(
								'module' => $module,
								'select' => '`object`.`id`, `object`.`title`, `object`.`catalogueid`, `object`.`slug`, `object`.`canonical`, `object`.`image`, `object`.`description`, `object`.`created`'.(($module == 'product') ? ', `object`.`price`, `object`.`price_sale`, `object`.`code`' : '').(($module == 'media') ? ', `object`.`video_link`, `object`.`video_iframe`' : ''),
...
if(!function_exists('form_render')){
	function form_render($param = ''){
		$html = '';
		$html .= '<form class="uk-form form ajax-form loading" method="" action="" data-validate="'.base64_encode(json_encode($param['data'])).'">';
			$html .= '<div class="error hidden">';
				$html .= '<div class="alert alert-danger"></div>';
			$html .= '</div><!-- /.error -->';
			$html .= '<div class="bg-loader"></div>';
			if(isset($param['data']) && is_array($param['data']) && count($param['data'])){
				foreach($param['data'] as $key => $val){
					if($val['type'] == 'textarea'){
						$html = $html.'<div class="form-row"><textarea  name="'.$val['name'].'" placeholder="'.$val['label'].'" value="'.((isset($val['value']) && $val['value'] != '')?  $val['value']:'').'"'.(($val['rule'] != '')? 'required':'').' class="textarea"></textarea></div>';
					}else{
						$html .= '<div class="form-row"><input type="'.$val['type'].'" name="'.$val['name'].'" class="input-text" placeholder="'.$val['label'].'" value="'.((isset($val['value']) && $val['value'] != '')?  $val['value']:'').'"'.(($val['rule'] != '')? 'required':'').' ></div>';
					}
				}
			}
			$html .= '<div class="form-row"><button type="submit" name="submit" value="" class="btn-submit">'.$param['submitText'].'</button></div>';
		$html .= '</form>';
		return $html;
	}
}
