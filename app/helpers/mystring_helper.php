...
if(!function_exists('imagecached')){
	function imagecached($src = ''){
		$path = substr(APPPATH, 0, -4);
		require_once $path.'vendor/nielse63/phpimagecache/src/ImageCache/ImageCache.php';
		$imagecache = new ImageCache();
	 	$imagecache->cached_image_directory = $path . '/images/cached';
	 	$cached_src = $imagecache->cache($path.$src);
		return 'images/cached/'.basename($imagecache->cached_filename);
	}
}
