...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, icon, publish, created, order, level, lft, rgt, (SELECT fullname FROM user WHERE user.id = product_catalogue.userid_created) as user_created',
				'table' => 'product_catalogue',
	...
	public function Update($id = 0){
		...
		$detailCatalogue = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description, meta_title, meta_description, parentid, image, image_json, publish, brand_json, icon,article_json',
			'table' => 'product_catalogue',
			'where' => array('id' => $id),
		));
