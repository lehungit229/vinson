	...
	public function view($page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$data['listData'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title,canonical, image, publish, order, price, price_sale, quantity_cuoi_ki, quantity_dau_ki, catalogue, (SELECT account FROM user WHERE user.id = product.userid_created) as user_created, version, viewed, catalogueid, (SELECT title FROM product_catalogue WHERE product_catalogue.id = product.catalogueid) as catalogue_title, price_contact',
				'table' => 'product',
				'query' => $query,
