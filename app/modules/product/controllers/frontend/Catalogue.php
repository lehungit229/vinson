...
class Catalogue extends MY_Controller {
	...
	public function view($catalogueid = 0, $page = 1){
        ...
        $detailCatalogue = $this->Autoload_Model->_get_where(array(
            'select' => 'id, title, level, lft, rgt, parentid, brand_json, image_json, attrid, canonical,description, image, icon, meta_title, meta_description',
        ...
        if($config['total_rows'] > 0){
            ...
            $productList = $this->Autoload_Model->_get_where(array(
                'select' => 'tb1.id, tb1.id as productid, tb1.title, tb1.canonical, tb1.price, tb1.price_sale, tb1.price_contact, tb1.image, tb1.version_json, tb1.image_color_json, tb2.end_date, tb1.description, (SELECT title FROM product_catalogue WHERE product_catalogue.id = tb1.catalogueid) as cat_title',
                'table' => 'product as tb1',
                'where' => array('tb1.publish' => 0),
        ...
        $data['children'] = $this->Autoload_Model->_get_where(array(
