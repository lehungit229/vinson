...
class Article extends MY_Controller {
		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				$data['listArticle'] = $this->Autoload_Model->_condition(array(
					'module' => 'article',
					'select' => '`object`.`id`, `object`.`title`, `object`.`slug`,`object`.`canonical`, `object`.`catalogueid`, `object`.`catalogue`, `object`.`publish`, `object`.`image`, `object`.`created`, `object`.`order`, `object`.`viewed`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM article_catalogue WHERE article_catalogue.id = object.catalogueid) as catalogue_title',
					'where' => ((!empty($extend)) ? '`object`.`userid_created` = '.$this->auth['id'].'' : ''),
					'keyword' => '(`object`.`title` LIKE \'%'.$keyword.'%\' AND `object`.`description` LIKE \'%'.$keyword.'%\')',
					'catalogueid' => $catalogueid,
					'limit' => $perpage,
				...
				$data['listArticle'] = $this->Autoload_Model->_get_where(array(
					'select' => 'id, catalogueid, catalogue, title, canonical, publish, created, order, viewed, image, (SELECT fullname FROM user WHERE user.id = article.userid_created) as user_created, (SELECT title FROM article_catalogue WHERE article_catalogue.id = article.catalogueid) as catalogue_title',
					'table' => 'article',
					'where_extend' => $extend,
	...
	public function Update($id = 0){
		...
		$detailArticle = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description, meta_title, catalogueid, catalogue, tag, meta_description, image, publish, publish_time, amp',
			'table' => 'article',
			'where' => array('id' => $id),
		));
