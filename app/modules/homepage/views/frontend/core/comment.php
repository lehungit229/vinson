<div class="block-comments">
					...
					<div class="uk-grid uk-grid-small uk-grid-width-large-1-2">
						...
						<div class="comment-infomation">
							<div class="uk-grid uk-grid-small uk-grid-width-large-1-2 mb10">
								...
								<div class="form-row">
									<?php echo form_input('comment_phone', htmlspecialchars_decode(html_entity_decode(set_value('comment_phone'))), 'placeholder="Số Điện thoại" class="input-text cmt-phone" autocomplete="off"');?>
								</div>
							</div>
							<div class="uk-grid uk-grid-small uk-grid-width-large-1-2 mb10">
								<div class="form-row">
									<?php echo form_input('comment_email', htmlspecialchars_decode(html_entity_decode(set_value('comment_email'))), 'placeholder="Email" class="input-text cmt-email" autocomplete="off"');?>
								</div>
								<div class="btn-cmt sent-cmt">
									<div class="loading">
										<div class="bg-loader"></div>
										<button type="submit" name="sent_comment" value="sent_comment" class="btn btn-success btn-submit comment-btn uk-width-1-1" data-module="<?php echo $module;?>" data-detailid="<?php echo $moduleid;?>">Gửi</button>
									</div>
								</div>
							</div>
	...
	<div class="ibox-content" style="position:relative;padding-right:10px;padding-left:10px;">
		<?php if($this->permissionComment == 'admin'){ ?>
			<?php $this->load->view('homepage/frontend/core/html_comment_admin'); ?>
		<!-- end comment-admin -->
		<?php }else{ ?>
			<?php $this->load->view('homepage/frontend/core/html_comment_client'); ?>
		<!-- chưa đăng nhập -->
		<?php } ?>
	</div>
</div>
...
$(document).ready(function(){
	...
	$(document).on('change','.publish',function(){
		...
		let formURL = 'comment/ajax/comment/status';
			$.post(formURL, {
				objectid: objectid},
				function(data){
	...
	$(document).on('click' , '#form-front-comment .sent-cmt .btn-submit', function(){
		...
		let cmtPhone = formCmt.find('.cmt-phone').val();
		let cmtEmail = formCmt.find('.cmt-email').val();
		...
		time = setTimeout(function(){
			...
			$.ajax({
				method: "POST",
				url: ajaxUrl,
				data: {param: param, cmtName: cmtName, cmtPhone: cmtPhone, cmtEmail: cmtEmail, cmtContent: cmtContent,},
				dataType: "json",
				cache: false,
				success: function(json){
					loader.hide();
					if(json.error != ''){
						formCmt.find('.success').addClass('hidden');
						...
						formCmt.find('.input-text').val('');
						formCmt.find('.textarea').val('');
					}
				}
			});
		} , 300);
	...
	$(document).on('click' , '.loadmore-cmt .btn-loadmore' , function(){
		let _this = $(this);
		let limit =  parseInt(_this.attr('data-limit'));
		let start =  parseInt(_this.attr('data-start')) * parseInt(_this.attr('data-limit'));
		let permissionComment =  _this.attr('data-permissionComment');
		let param = {
			'module': _this.attr('data-module'),
			'detailid': _this.attr('data-detailid'),
			'start': start,
		...
		// console.log(totalComment); return false;
...
function openKCFinderThumb(button) {
	window.KCFinder = {
		callBackMultiple: function(files) {
			...
			console.log($('.lightBoxGallery img'));
		...
		comment += '<div class="box-comment box-reply loading" style="margin-top: 10px;">';
			...
			comment += '<form action="" class="form uk-form uk-clearfix">';
				...
				comment += '<div class="uk-flex uk-flex-middle uk-flex-space-between mt5">';
					...
					comment += '</div>';
					comment += '<div class="btn-cmt sent-cmt"><button type="submit" name="sent_comment" value="sent_comment" class="btn-success btn-submit" data-parentid = '+param.id+' data-module = '+param.module+' data-detailid = '+param.detailid+' >Gửi</button></div>';
				comment += '</div>';
			comment += '</form>';
		comment += '</div>';
