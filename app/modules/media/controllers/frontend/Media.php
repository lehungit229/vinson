...
class Media extends MY_Controller {
	...
	public function view($id = 0){
		...
		$detailMedia = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, catalogueid, description, image, viewed, (SELECT fullname FROM user WHERE user.id = media.userid_created) as fullname, created, image_json',
			'table' => 'media',
			'where' => array('id' => $id),
		));
